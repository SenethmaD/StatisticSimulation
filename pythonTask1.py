...
def run_simulation():
    ...
    for num_darts in num_darts_list:
        ...
        # Create a new worksheet for each num_darts
        ws = wb.create_sheet(title=f'Num Darts {num_darts}')
        ws.append(['Num Darts', 'Mean Pi', 'Mode Pi'])
        for result in experiment_results:
            ws.append([result['Num Darts'], result['Mean Pi'], result['Mode Pi']])
    # Remove the default sheet created by openpyxl
    wb.remove(wb['Sheet'])
    # Save the workbook
    wb.save('dart_simulation_results.xlsx')
...
if __name__ == "__main__":
    ...
    wb = openpyxl.load_workbook('dart_simulation_results.xlsx')
    for sheet_name in wb.sheetnames:
        if sheet_name.startswith('Num Darts'):
            df = pd.read_excel('dart_simulation_results.xlsx', sheet_name=sheet_name)
            plot_results_and_save(df, sheet_name, wb)
    wb.save('dart_simulation_results_with_plots.xlsx')